...
export const querypaymentrunsEndpoint: ApiEndpoint = {
  ...
  "queryParams": [
    ...
    },
    {
      ...
      "type": "array",
      "required": false,
      "description": "A case-insensitive query parameter that specifies the sort order of the list, which can be either ascending (e.g. `accountnumber.ASC`) or descending (e.g. `accountnumber.DESC`). You cannot sort on properties in arrays. If the array-type properties are specified for the `sort[]` parameter, they are ignored. Supported sortable fields: - id - updateddate - executeddate - numberofpayments - paymentrunnumber - status - targetdate",
      "itemType": "string"
    },
    {
      "name": "expand[]",
      ...
      "type": "array",
      "required": false,
      "description": "Allows you to expand responses by including related object information in a single call. The Payment Run object does not support expanding any related objects.",
      "itemType": "string"
    },
    {
      "name": "filter[]",
      ...
      "type": "array",
      "required": false,
      "description": "A case-insensitive filter on the list. Supported filterable fields: - id - updateddate - executeddate - numberofpayments - paymentrunnumber - status - targetdate - {indexedcustomfield}: Use the format like `customField__c` to filter on custom fields.",
      "itemType": "string"
    },
    {
      "name": "fields[]",
